...
@section('container.main')
    ...
    <section class="lg:px-14 px-6 pt-[70px] pb-[40px] lg:py-[100px]">
        <div class="grid lg:grid-cols-4 grid-cols-2 auto-rows-[120px] lg:auto-rows-[380px] gap-3 lg:gap-6">
            @foreach ($gallery as $key => $value)
                @if (($key + 1) % 3  == 0)
                    <div class="col-span-2">
                        <img class="h-full w-full rounded-2xl object-cover cursor-pointer"
                            src="{{ url('uploads/' . $value->image) }}" alt="" data-gallery="gallery1"
                            data-index="{{ $key }}" />
                    </div>
                @else
        ...
        <div id="videoSlider" class="swiper mx-auto rounded-[16px]">
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <img src="{{ url('uploads/' . $videos->image) }}"
                        class="h-[250px] gallery-img lg:h-[500px] sliderImage w-full object-cover rounded-[16px]"
                        data-index="0" alt="{{$videos->alt}}" />
                    <i
                        class="fa-solid fa-play absolute cursor-pointer top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex justify-center text-[18px] items-center text-white bg-[#63636394] h-[65px] w-[65px] rounded-full"></i>
                </div>
                @endforeach
    ...
    <section class="lg:px-14 px-6 lg:py-[100px] py-[60px]">
        {{-- <div class="grid lg:grid-cols-4 grid-cols-2 auto-rows-[120px] lg:auto-rows-[380px] gap-3 lg:gap-6">
            <div class="col-span-1">
                <img class="h-full w-full rounded-2xl object-cover cursor-pointer" src="./images/2.jpg" alt=""
                    data-gallery="gallery2" data-index="0" />
            </div>

            <div class="col-span-1">
                <img class="h-full w-full rounded-2xl object-cover cursor-pointer" src="./images/10.jpg" alt=""
                    data-gallery="gallery2" data-index="1" />
            </div>

            <div class="col-span-2">
                <img class="h-full w-full rounded-2xl object-cover cursor-pointer" src="./images/gallery-banner.png"
                    alt="" data-gallery="gallery2" data-index="2" />
            </div>

            <div class="col-span-1">
                <img class="h-full w-full rounded-2xl object-cover cursor-pointer" src="./images/11.jpg" alt=""
                    data-gallery="gallery2" data-index="3" />
            </div>

            <div class="col-span-1">
                <img class="h-full w-full rounded-2xl object-cover cursor-pointer" src="./images/12.jpg" alt=""
                    data-gallery="gallery2" data-index="4" />
            </div>

            <div class="col-span-2">
                <img class="h-full w-full rounded-2xl object-cover cursor-pointer" src="./images/13.jpg" alt=""
                    data-gallery="gallery2" data-index="5" />
            </div>
        </div> --}}
    </section>

    <div id="imageModal" class="hidden fixed inset-0 bg-black/80 flex items-center justify-center z-[9999]">
        <button class="absolute top-5 right-5 text-white text-3xl" onclick="closeModal()">
            ✕
        </button>

        <div class="relative w-[90%] lg:w-[60%]">
            <img id="modalImage" class="w-full h-auto rounded-2xl object-contain" src="" alt="Modal Image" />

            <button onclick="changeSlide(-1)" class="absolute top-1/2 left-2 -translate-y-1/2 text-white text-4xl">
                ‹
            </button>

            <button onclick="changeSlide(1)" class="absolute top-1/2 right-2 -translate-y-1/2 text-white text-4xl">
                ›
            </button>
        </div>
    </div>
